...
export function HeroCarousel({ slides = [], snapshotDate = null, onOpenSlide }) {
  ...
  const safeSlides = Array.isArray(slides) ? slides.filter((s) => s?.title && (s.backdropUrl || s.posterUrl)) : []
  const slide = safeSlides[index] ?? null
  ...
  return (
    ...
    >
      <div className="relative min-h-[22rem] md:min-h-[28rem]">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-[var(--bg-app)] via-transparent to-black/30" />

        <div className="relative flex min-h-[22rem] flex-col justify-end px-5 pb-10 pt-16 md:min-h-[28rem] md:px-10 md:pb-12 md:pt-20">
          {slide.badge && (
            <span className="mb-3 inline-flex w-fit rounded-md bg-[var(--accent-red)] px-2.5 py-1 text-xs font-bold tracking-wide text-white">
              {slide.badge}
            </span>
          )}
          ...
          {safeSlides.length > 1 && (
            <div className="mt-6 flex items-center gap-2" role="tablist" aria-label="Slides hero">
              {safeSlides.map((item, i) => (
                <button
                  key={`${item.slot ?? i}-${item.title}`}
                  type="button"
                  role="tab"
