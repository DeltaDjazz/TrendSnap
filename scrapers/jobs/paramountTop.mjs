...
const TOP_N = 10;
const FLIXPATROL_PLATFORM = 'paramount-plus';
...
function flixpatrolTop10Url(date) {
    return `https://flixpatrol.com/top10/${FLIXPATROL_PLATFORM}/france/${date}/`;
}
...
async function run() {
    ...
    try {
        ...
        console.log(`FlixPatrol Paramount+ (${usedDate}) : ${series.length} séries`);

        if (series.length === 0) {
            throw new Error('Impossible de récupérer le TOP 10 séries FlixPatrol Paramount+.');
        }

        const enrichedSeries = await enrichList(series, 'tv', auth, page);
        saveSnapshot('paramount-series.json', enrichedSeries);

        console.log('Job Paramount+ FlixPatrol + TMDB terminé.');
    } catch (error) {
        console.error('Erreur job Paramount+ :', error.message);
        process.exitCode = 1;
    } finally {
        if (browser) {
            await browser.close();
